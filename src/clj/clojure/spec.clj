...
(defn conform
  ...
  [spec x]
  (let [ret (conform* (specize spec) x)]
    (if (invalid?* ret) ::invalid ret)))
...
(defn- call-valid?
  [f specs args]
  (let [cargs (conform* (specize (:args specs)) args)]
    (when-not (invalid?* cargs)
      (let [ret (apply f args)
            cret (conform* (specize (:ret specs)) ret)]
